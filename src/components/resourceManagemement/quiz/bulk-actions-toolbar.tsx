...
import { useDeleteQuiz, usePatchUpdateQuizStatus } from "@/lib/api/mutations";
...
export function BulkActionsToolbar({
  ...
  const handleDelete = async () => {
    ...
    try {
      const result = await deleteQuiz({ quizIds: selectedIds });
  ...
  const handleStatusUpdate = async (
    ...
    setIsProcessing(true);
    try {
      const result = await updateQuizStatus({ quizIds: selectedIds, status });
      if (result.status === 200) {
        toast.success(result.data.message);
        onClearSelection();
        onComplete();
