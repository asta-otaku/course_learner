  ...
  const handleUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      if (!fileExt || !allowedTypes.includes(fileExt.toLowerCase())) {
        throw new Error('Please select a valid video file (MP4, WebM, OGG, MOV, or AVI)')
      }
      ...
      console.log('Uploading video:', { fileName, filePath, userId: user.id })
      ...
      if (uploadError) {
        ...
        if (uploadError.message?.includes('row-level security') || uploadError.message?.includes('RLS')) {
          throw new Error('Storage permissions not configured. Please ensure RLS policies are set up for lesson-videos bucket.')
        } else if (uploadError.message?.includes('not found')) {
          throw new Error('Storage bucket "lesson-videos" not found. Please create it in Supabase dashboard.')
        }
