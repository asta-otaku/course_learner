...
import { useGetFolders, fetchQuestions } from "@/lib/api/queries";
...
export function QuestionsWithFolders({
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- counts/subfolders refresh when folder list identity changes
  }, [folders]);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- breadcrumb follows selected folder
  ...
  const handleFolderSelect = async (folderId: string | null) => {
    ...
    try {
      ...
      const questionsResult = await fetchQuestions({
        ...
        type: searchParams.get("type")?.split(",") as string[],
        sortBy: searchParams.get("sortBy") || "created_at",
        sortOrder: (searchParams.get("sortOrder") as "ASC" | "DESC") || "DESC",
      });

      setQuestions(questionsResult.questions as unknown as Question[]);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleFolderRename = async (_folderId: string, _newName: string) => {
    try {
