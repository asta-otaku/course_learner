...
const MessagingPlatform = () => {
  ...
  const { data: tutorChatListData } = useGetTutorChatList(isTutorMode);
  const { data: studentChatListData } = useGetStudentChatList({
    childId: isTutorMode ? "" : activeProfile?.id || "",
  });
  const chatListData = isTutorMode ? tutorChatListData : studentChatListData;
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- pagination uses latest limit/page from the messages query
  }, [messagesData]);
  ...
  const handleSendMessage = useCallback(
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- display names are read at send time
    [
      newMessage,
      activeChat,
      isTutorMode,
