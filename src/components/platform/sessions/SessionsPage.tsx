...
function Sessions() {
  ...
  const _availableTutors = useMemo(() => {
  ...
  const _availablePaginationInfo = useMemo(
    () => ({
      page: availablePagination.page,
      limit: availablePagination.limit,
  ...
  const _availableSessions = useMemo(() => {
    if (!availableSessionsData?.data?.data) return [];
  ...
  const _handleBookedLimitChange = (limit: number) => {
    setBookedPagination((prev) => ({ ...prev, limit, page: 1 }));
  };

  const _handleAvailablePageChange = (page: number) => {
    setAvailablePagination((prev) => ({ ...prev, page }));
  };

  const _handleAvailableLimitChange = (limit: number) => {
